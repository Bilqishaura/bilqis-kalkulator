...
class Kalkulator:
    def __init__(self, root):
        ...
        self.display = tk.Entry(root, font=("Segoe UI", 20), justify='right',
        ...
        for r, row in enumerate(tombol):
            for c, val in enumerate(row):
                btn = tk.Button(tombol_frame, text=val, font=("Segoe UI", 16),
                                command=lambda v=val: self.klik(v),
    ...
    # ======================
    #   FUNGSI PERSEN PINTAR
    # ======================
    def hitung_persen(self):
        eksp = self.display.get()

        # Mode standar → langsung angka/100
        if eksp.isdigit():
            hasil = float(eksp) / 100
            self.display.delete(0, tk.END)
            self.display.insert(tk.END, hasil)
            return
        ...
        # Mode gagal → standar aja
        try:
            angka = float(eksp) / 100
            self.display.delete(0, tk.END)
            self.display.insert(tk.END, angka)
        except:
            self.display.delete(0, tk.END)
            self.display.insert(tk.END, "Error")
